...
function fiftyfiftychance() {
  ...
  fiftyButton.addEventListener("click", (el) => {
    ...
    question1.forEach((el) => {
      if (
        el.question.replace(/[^a-zA-Z]/g, "") ===
        curentQuestion.replace(/[^a-zA-Z]/g, "")
      ) {
        //replace, panaikina nereikalingus zenklus pries lygininat
    ...
    const randomWrongAnswer = Math.floor(
      Math.random() * wrongAnswerArray.length
    );
